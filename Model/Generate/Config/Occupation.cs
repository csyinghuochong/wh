...
namespace ET
{
    [ProtoContract]
    [Config]
    public partial class OccupationCategory : ProtoObject, IMerge
    {
        public static OccupationCategory Instance;
		
        [ProtoIgnore]
        [BsonIgnore]
        private Dictionary<int, Occupation> dict = new Dictionary<int, Occupation>();
		
        [BsonElement]
        [ProtoMember(1)]
        private List<Occupation> list = new List<Occupation>();
        ...
        {
            OccupationCategory s = o as OccupationCategory;
            this.list.AddRange(s.list);
        }
		
        public override void EndInit()
        {
            foreach (Occupation config in list)
            {
                config.EndInit();
        ...
        {
            this.dict.TryGetValue(id, out Occupation item);

            if (item == null)
            {
                throw new Exception($"配置找不到，配置表名: {nameof (Occupation)}，配置id: {id}");
            }
        ...
        public Dictionary<int, Occupation> GetAll()
        {
            return this.dict;
        }

        public Occupation GetOne()
        {
    ...
    [ProtoContract]
	public partial class Occupation: ProtoObject, IConfig
	{
		/// <summary>Id</summary>
		[ProtoMember(1)]
