...
namespace ET
{
    ...
    public static class DeleteZoneHelper
    {
        public static async ETTask DeletionZone(int zone)
        {
            ...
            //中心区
            startZoneConfig = StartZoneConfigCategory.Instance.Get(ComHelp.CenterZoneId);
            Game.Scene.GetComponent<DBComponent>().InitDatabase(startZoneConfig);
            ...
            {
                //删除的账号在中心区做个记录
                List<DBCenterAccountInfo> dBAccountInfos = await Game.Scene.GetComponent<DBComponent>().Query<DBCenterAccountInfo>(ComHelp.CenterZoneId, d => d.Id == entity.Id);
                ...
                await Game.Scene.GetComponent<DBComponent>().Save<DBCenterAccountInfo>(ComHelp.CenterZoneId, dBAccountInfos[0]);
            }
