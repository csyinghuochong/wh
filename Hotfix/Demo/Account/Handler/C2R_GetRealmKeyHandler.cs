...
namespace ET
{
    public class C2R_GetRealmKeyHandler : AMRpcHandler<C2R_GetRealmKey, R2C_GetRealmKey>
    {
        protected override async ETTask Run(Session session, C2R_GetRealmKey request, R2C_GetRealmKey response, Action reply)
        {
            try
            {
                if (session.DomainScene().SceneType != SceneType.Realm)
                {
                    Log.Warning($"LoginTest 请求的Scene错误，当前Scene为：{session.DomainScene().SceneType}  {session.RemoteAddress}");
                    session.Dispose();
                    return;
                }
                ...
                using (session.AddComponent<SessionLockingComponent>())
                {
                    ...
                    {
                        StartSceneConfig realmStartSceneConfig = RealmGateAddressHelper.GetRealm(session.DomainZone());
