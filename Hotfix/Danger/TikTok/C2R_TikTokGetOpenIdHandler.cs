...
namespace ET
{
    ...
    //抖音官包才会调用
    [MessageHandler]
    public class C2R_TikTokGetOpenIdHandler : AMRpcHandler<C2R_TikTokGetOpenId, R2C_TikTokGetOpenId>
    {
        protected override async ETTask Run(Session session, C2R_TikTokGetOpenId request, R2C_TikTokGetOpenId response, Action reply)
        {
            ...
            */
            ...
            if (centerAccount.PlayerInfo == null && !string.IsNullOrEmpty(sdk_open_id))
            {
                ...
                R2Other_TiktokBinging centerAccountbing = (R2Other_TiktokBinging)await ActorMessageSenderComponent.Instance.Call(accountZone, new Other2R_TiktokBinging()
                {
                    Account = sdk_open_id,
