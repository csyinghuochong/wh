...
namespace ET
{
    ...
    public class C2M_TakeOutAllHandler: AMActorLocationRpcHandler<Unit, C2M_TakeOutAllRequest, M2C_TakeOutAllResponse>
    {
        protected override async ETTask Run(Unit unit, C2M_TakeOutAllRequest request, M2C_TakeOutAllResponse response, Action reply)
        {
            ...
            int leftBagSpace = unit.GetComponent<BagComponentServer>().GetBagLeftCell();
            if (leftBagSpace <= 0)
            {
            ...
            // 要值传递!!!，上课教的都忘了
            storeLists.AddRange(unit.GetComponent<BagComponentServer>().GetItemByLoc((ItemLocType)hourseId));

            for (int i = 0; i < storeLists.Count; i++)
            {
                unit.GetComponent<BagComponentServer>().OnChangeItemLoc(storeLists[i], ItemLocType.ItemLocBag, (ItemLocType)hourseId);
