    ...
    public class C2M_ItemXiLianHandler : AMActorLocationRpcHandler<Unit, C2M_ItemXiLianRequest, M2C_ItemXiLianResponse>
    {
        ...
        {            response.Error = ErrorCode.ERR_ModifyData;
            ...
            try
            {
                ...
                if (bagInfo == null)
                {
                    bagInfo = bagComponentServer.GetItemByLoc(ItemLocType.ItemLocEquip, request.OperateBagID);
                    //itemLocType = ItemLocType.ItemLocEquip;
                }
                if (bagInfo == null)
                {
                    /*bagInfo = bagComponentServer.GetItemByLoc(ItemLocType.ItemLocEquip_2, request.OperateBagID); 
               ...
               int equipType = ItemHelper.GetNewEquipType(bagInfo);
                ...
                int[] costItems = null;//ldItem.XiLianStone;
                ...
                bool sucess = bagComponentServer.OnCostItemData(rewardItems, ItemLocType.ItemLocBag, ItemGetWay.ItemXiLian);
                if (!sucess)
                {
                    reply();
                    return;
                ...
                //unit.GetComponent<RoleDailyDataComponentServer>()?.AddNowXiLian();
                reply();
                await ETTask.CompletedTask;
            }
            catch (Exception ex)
