...
namespace ET
{
    ...
    public class C2M_EquipmentIncreaseHandler: AMActorLocationRpcHandler<Unit, C2M_EquipmentIncreaseRequest, M2C_EquipmentIncreaseResponse>
    {
        ...
        {            response.Error = ErrorCode.ERR_ModifyData;
            ...
            BagInfo equipmentBagInfo = bagComponentServer.GetItemByLoc(ItemLocType.ItemLocBag, request.EquipmentBagInfo.BagInfoID);
            if(equipmentBagInfo == null)
            {
                equipmentBagInfo = unit.GetComponent<BagComponentServer>().GetItemByLoc(ItemLocType.ItemLocEquip, request.EquipmentBagInfo.BagInfoID);
            }
            ...
            // 消耗卷轴
            bagComponentServer.OnCostItemData(reelBagInfo.BagInfoID, 1);
