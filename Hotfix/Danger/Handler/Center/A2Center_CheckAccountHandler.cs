        ...
        protected override async ETTask Run(Scene scene, A2Center_CheckAccount request, Center2A_CheckAccount response, Action reply)
        {
            ...
            if (response.PlayerInfo!= null &&  !string.IsNullOrEmpty(request.OAID)  && !request.OAID.Contains("00000000"))
            {
                ...
                if (centerDataCaches != null && centerDataCaches.Count > 0 && downloadtype > 0)
                {
                    ...
                    Game.Scene.GetComponent<DBComponent>().Save(ComHelp.CenterZoneId, dBCenterTaprep).Coroutine();
                }
            }
            ...
            if (request.DeviceID != "35c3d38dee2d064f4e77767b8a9ef4d3d7353e36" && !request.AccountName.Contains("testcn"))
            {
                ...
                if (dBCenterAccountInfo != null && !string.IsNullOrEmpty(request.DeviceID) && dBCenterAccountInfo.DeviceID != request.DeviceID)
                {
                    ...
                    await Game.Scene.GetComponent<DBComponent>().Save<DBCenterAccountInfo>(ComHelp.CenterZoneId, dBCenterAccountInfo);
