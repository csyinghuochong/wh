...
namespace ET
{
    ...
    [MessageHandler]
    public class C2R_DeleteAccountHandler : AMRpcHandler<C2R_DeleteAccountRequest, R2C_DeleteAccountResponse>
    {
        protected override async ETTask Run(Session session, C2R_DeleteAccountRequest request, R2C_DeleteAccountResponse response, Action reply)
        {
            ...
            if (dBCenterAccountInfo != null)
            {
                ...
                { 
                    ...
                    await Game.Scene.GetComponent<DBComponent>().Remove<DBAccountBagInfo>(pzone, dBCenterAccountInfo.Id);
                }
            }
            else
            {
