...
namespace ET
{
    ...
    public class Skill_Com_Summon_3: SkillHandler
    {
        ...
        public override void OnExecute()
        {
            ...
            RoleInfo roleInfo = theUnitFrom.GetComponent<RoleInfoComponentServer>()?.RoleInfo;
            if (roleInfo != null && roleInfo.DefeatedBossIds.Count > 0)
            {
                int monsterId = 0;
                float range = 0f;
                int number = 0;

                monsterId = CommonConfig.DefeatedBossIds[roleInfo.DefeatedBossIds[RandomHelper.RandomNumber(0, roleInfo.DefeatedBossIds.Count)]];
