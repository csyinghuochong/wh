...
namespace ET
{
    ...
    public class C2M_GemHeChengQuickHandler : AMActorLocationRpcHandler<Unit, C2M_GemHeChengQuickRequest, M2C_GemHeChengQuickResponse>
    {
        protected override async ETTask Run(Unit unit, C2M_GemHeChengQuickRequest request, M2C_GemHeChengQuickResponse response, Action reply)
        {
            ...
            {
                leftCell = unit.GetComponent<BagComponentServer>().GetBagLeftCell();
            }
            else
            {
                leftCell = unit.GetComponent<BagComponentServer>().GetHourseLeftCell(request.LocType);
            }

            List<BagInfo> bagItemList = unit.GetComponent<BagComponentServer>().GetItemByLoc((ItemLocType)request.LocType);
            ...
            int needCell = unit.GetComponent<BagComponentServer>().GetNeedCell(rewardItems, (ItemLocType)request.LocType);
            if (needCell < 1)
            {
                needCell = 1;
            }
            ...
            if (removeItems != string.Empty)
            {
                unit.GetComponent<BagComponentServer>().OnCostItemData(removeItems, (ItemLocType)request.LocType, ItemGetWay.GemHeCheng);
            }

            if (rewardItems.Count > 0)
            {
                bool ret = unit.GetComponent<BagComponentServer>().OnAddItemData(rewardItems, string.Empty, $"{ItemGetWay.GemHeCheng}_{TimeHelper.ServerNow()}",
