...
namespace ET
{
    ...
    public class C2M_LingDiRewardHandler : AMActorLocationRpcHandler<Unit, C2M_LingDiRewardRequest, M2C_LingDiRewardResponse>
    {
        protected override async ETTask Run(Unit unit, C2M_LingDiRewardRequest request, M2C_LingDiRewardResponse response, Action reply)
        {
            ...
            if (!unit.GetComponent<BagComponentServer>().OnCostItemData($"{config.BuyItemID};{config.BuyPrice}", ItemLocType.ItemLocBag, ItemGetWay.LingDiReward))
            {
                response.Error = ErrorCode.ERR_ItemNotEnoughError;
                reply();
                return;
            ...
            unit.GetComponent<BagComponentServer>().OnAddItemData($"{config.ItemID};1", $"{ItemGetWay.LingDiReward}_{TimeHelper.ServerNow()}");
 */
            reply();
